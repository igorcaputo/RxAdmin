...
procedure TfrmRelatorio1.btnBuscaClick(Sender: TObject);
var
...
procedure TfrmRelatorio1.onExit(Sender: TObject);
begin
end;

procedure TfrmRelatorio1.onShow(Sender: TObject);
var
...
procedure TfrmRelatorio1.FrameResize(Sender: TObject);
begin
  btnImprimir.Left := (frmPrincipal.fraContainer.Width - btnImprimir.Width) div 2;
end;

procedure TfrmRelatorio1.btnImprimirClick(Sender: TObject);
var
...
begin
  ...
  app := 'C:\Mobb\Relatorio\';//ExtractFilePath(ParamStr(0));
{$IFDEF UNIX}
...
{$ENDIF}
  ...
  Dm.frReport1.LoadFromFile(app + 'SaldoSeparacao.lrf');

  Dm.frReport1.PrepareReport;
  //Synchronize(@Dm.frReport1.PrepareReport);
  Dm.frReport1.ExportTo(TFrTNPDFExportFilter, app + arqpdf);

  appFile := TFileStream.Create(app + arqpdf, fmOpenRead);
...
procedure TfrmRelatorio1.CriaLabel(ANome, ACaption, AVisible: string;
...
procedure TfrmRelatorio1.CriaBusca(ANome, AVisible, ABusca: string; var ATop: integer;
...
procedure TfrmRelatorio1.CriaComboBox(ANome, ACaption, AVisible: string;
  var ATop: integer; AParent: TWinControl);
var
...
procedure TfrmRelatorio1.CriaEdit(ANome, ACaption, AVisible: string;
...
procedure TfrmRelatorio1.CampoDE_ATE(ANome, AVisible: string;
  var ATop: integer; AParent: TWinControl);
var
...
constructor TfrmRelatorio1.Create(AOwner: TComponent);
//var
//  a: integer;
//  strCaption: string;
begin
...
finalization
  UnRegisterClass(TfrmRelatorio1);
end.
