...
type
  ...
  public
    constructor Create(AConnection: T1;
      const ATableName,AMaping: string); reintroduce; virtual;
    destructor Destroy; override;
  ...
  public
    constructor Create(AConnection: T1;
      const ATableName, AMaping: string); reintroduce; override;
    destructor Destroy; override;
...
constructor TdGOpf.Create(AConnection: T1; const ATableName,AMaping: string);
begin
  inherited Create(AConnection);
  FConnection := AConnection;
  FQuery := T2.Create(FConnection);
  FTable := CreateTable;
  FTable.Maping:=AMaping;
...
constructor TdGEntityOpf.Create(AConnection: T1; const ATableName, AMaping: string);
begin
  FEntity := CreateEntity;
  inherited Create(AConnection, ATableName, AMaping);
  FEntity := CreateEntity;
end;
