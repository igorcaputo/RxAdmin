...
uses
  dClasses, dUtils, Classes, SysUtils, TypInfo,LazStringUtils;
...
type
  ...
  private
    ...
    procedure SetMaping(const AValue: string);
  public
  ...
  published
    ...
    property Maping: string read FMaping write SetMaping;
  ...
  public
    ...
    class function getAValueOfDict(strDict, strAKey: string
      ): string; virtual;
    procedure Build(out ASql: string;
...
procedure TdGTable.SetMaping(const AValue: string);
begin
  if Trim(AValue) = '' then
    Exit;
  if AValue <> FMaping then
  begin
    FMaping := AValue;
    //StrLower(PChar(FName));
  end;
...
begin
  ...
  begin
    ...
    M := String(ATable.Maping);
    NN := getAValueOfDict(M,N);
    NNN := NN + ' as ' + String(N);
...
class function TdGSelectBuilder.getAValueOfDict(strDict, strAKey: string
  ): string;
var
  strlistOfItems: TStringList;
  s, sRes: string;
begin
  strlistOfItems := TStringList.Create;
  ExtractStrings([','], [','], pchar(trim(strDict)), strlistOfItems);

  for s in strlistOfItems do
  begin
    if lowercase(strAKey) = lowercase(SplitString(s, ':').Strings[0]) then
    begin
      sRes := SplitString(s, ':').Strings[1];
      break;
    end;
  end;
  strlistOfItems.Free;
...
{ TdGInsertBuilder }
class function TdGInsertBuilder.getAValueOfDict(strDict, strAKey: string
  ): string;
var
  strlistOfItems: TStringList;
  s, sRes: string;
...
begin
  ...
  begin
    ...
    P := ATable.PropList^[I]^.Name;
    N := ATable.PropList^[I]^.Name;
    M := ATable.Maping;
    N := getAValueOfDict(M,N);
...
class function TdGUpdateBuilder.getAValueOfDict(strDict, strAKey: string
  ): string;
var
  strlistOfItems: TStringList;
  s, sRes: string;
